...
function run_sysbench_test() {
  ...
  mkdir -p "$run_dir"
  cp -f /tmp/IPRA-exp/sysbench/usr/share/sysbench/*.lua .
  /tmp/IPRA-exp/sysbench/bin/sysbench "${test}" --table-size=${table_size} --num-threads=1 --rand-type=uniform --rand-seed=1 --db-driver=mysql \
    --mysql-db=sysbench --tables=1 --mysql-socket=/tmp/mysql.sock --mysql-user=root prepare
  {
    if [[ "${table_size}" -ge "1000000" ]]; then
      /tmp/IPRA-exp/sysbench/bin/sysbench "${test}" --table-size=${table_size} --tables=1 --num-threads=1 --rand-type=uniform --rand-seed=1 --db-driver=mysql \
